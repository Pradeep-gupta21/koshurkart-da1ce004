-- Migration: 20260718000006_deferred_return_lock.sql
-- Refactors optimistic locking on vendor returns to expect 'requested' instead of 'processing'.
...
BEGIN
  -- Optimistic lock check before acquiring the FOR UPDATE lock
  SELECT oi.return_status INTO _status
  FROM public.order_items oi
  WHERE oi.id = _order_item_id;

  IF _status <> 'requested' THEN
    RAISE EXCEPTION 'RETURN_NOT_PENDING' USING ERRCODE = 'P0001';
  END IF;
  ...
  IF _vendor_id IS NULL THEN
    RAISE EXCEPTION 'Order item not found' USING ERRCODE = 'P0001';
  END IF;
  ...
  IF _new_balance < 0 THEN
    RAISE EXCEPTION 'Insufficient funds to cover return debit' USING ERRCODE = 'P0001';
  END IF;
