-- ============================================================================
-- Migration: 20260804130000_fix_approve_return_intent_auth_resolution.sql
--
-- Purpose:
--   Fix the approve_return_intent RPC to resolve the vendor identity 
--   internally using auth.users.id, preventing FORBIDDEN errors for 
--   legitimate edge function calls.
-- ============================================================================
...
BEGIN
    ...
    -- Edge Functions. The RPC receives the authenticated auth.users.id, 
    -- resolves the canonical vendors.id internally, and performs ownership 
    -- verification against the resolved vendor ID.
    -- Zero-trust applies to the *data* in Section 2.
    ...
    -- 5. Vendor ownership verification
    ...
    -------------------------------------------------------------------------
EXCEPTION
    -- Bubble up transient locking/infrastructure failures for canonical retry semantics
    WHEN serialization_failure OR deadlock_detected OR lock_not_available THEN
        RAISE WARNING '[approve_return_intent] Retryable concurrency failure for order_item %: % (SQLSTATE: %)', p_order_item_id, SQLERRM, SQLSTATE;
        RAISE;
