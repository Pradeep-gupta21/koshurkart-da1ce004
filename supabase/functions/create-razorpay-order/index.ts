...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!orderId) {
      return respondWithError(new PaymentError(ErrorCategory.VALIDATION, ERROR_CODES.INTERNAL_ERROR, "orderId is required", false), { ...getCorsHeaders(req), "Content-Type": "application/json" });
    }
    ...
    if (!order) {
      return respondWithError(new PaymentError(ErrorCategory.NOT_FOUND, ERROR_CODES.INTERNAL_ERROR, "Order not found", false), { ...getCorsHeaders(req), "Content-Type": "application/json" });
    }
    ...
    if (!amount || amount <= 0) {
      return respondWithError(new PaymentError(ErrorCategory.VALIDATION, ERROR_CODES.INTERNAL_ERROR, "Invalid order amount", false), { ...getCorsHeaders(req), "Content-Type": "application/json" });
    }
    ...
    if (amountPaise < 100) {
      return respondWithError(new PaymentError(ErrorCategory.VALIDATION, ERROR_CODES.INTERNAL_ERROR, "Amount must be at least ₹1", false), { ...getCorsHeaders(req), "Content-Type": "application/json" });
    }
    ...
    if (!razorpayRes.ok) {
      ...
      return respondWithError(new PaymentError(ErrorCategory.GATEWAY_ERROR, ERROR_CODES.INTERNAL_ERROR, parsedError, false), { ...getCorsHeaders(req), "Content-Type": "application/json" });
    }
